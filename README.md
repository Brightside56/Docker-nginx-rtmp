...
Для того, чтобы запустить контейнер (в фоновом режиме) и забиндить для него соответствующие порты хоста, запустите следующую команду:
```
docker run -d -p 1935:1935 -p 8080:8080 brightside/nginx-rtmp
```
